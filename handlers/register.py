...
def register_handlers(dp: Dispatcher):
    from handlers.handlers import (
        ...
        status_posting, edit_status_module, any_post, start_link_post, status_spam, edit_status_spam, 
        handle_caption_post, caption_text_post, subsupdate_handler, log_file_handler, activate_forward
    )
    from .admin.admin_handlers import request_to_join, handle_admin_response, join_member

    router = Router()

    router.message.register(activate_forward, Command(commands=['forwardActive']))
    ...
    router.message.register(caption_text_post, StateFilter(waiting.caption_post))
